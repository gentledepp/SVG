...
namespace Svg.Editor.Avalon.Views.CustomGestureRecognizer
{
    ...
    public class ZoomGestureRecognizer : GestureRecognizer
    {
        ...
        private const double ZOOM_DAMPING_FACTOR = 0.7; // Reduces zoom sensitivity
        ...
        public const double ZOOM_THRESHOLD = 10.0; // 10 pixel tolerance
        ...
        protected override void PointerMoved(PointerEventArgs e)
        {
            if (Target is Visual visual)
            {
                ...
                if (_firstContact != null && _secondContact != null)
                {
                    ...
                    // zoom
                    var scale = distance / _lastDistance;

                    // distance did not change, dont zoom
                    if (Math.Abs(distance - _lastDistance) < ZOOM_THRESHOLD)
                    {
                        return;
                    }
        ...
        protected override void PointerPressed(PointerPressedEventArgs e)
        {
            if (Target is Visual visual && (e.Pointer.Type == PointerType.Touch || e.Pointer.Type == PointerType.Pen))
            {
                ...
                if (_firstContact != null && _secondContact != null)
                {
                    ...
                    _origin = new Point((_firstPoint.X + _secondPoint.X) / 2.0f,
                        (_firstPoint.Y + _secondPoint.Y) / 2.0f);
        ...
        private static double GetAngleDegreeFromPoints(Point a, Point b)
        {
            ...
            var degree =
                rad * (180 / Math.PI) + 180; // Atan2 returns a radian value between -π to +π, in degrees -180 to +180.
